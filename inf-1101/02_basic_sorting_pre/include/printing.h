...
// All messages are printed to stderr, so data can be printed to stdout and redirected to files.
 ...
 */
#if LOG_LEVEL <= 0
#define INFO_PRINT(...) do { fprintf(stderr, "%s", BGRN); fprintf(stderr, "[INFO][%s %d]: ", __FILE__, __LINE__); fprintf(stderr, "%s", reset); fprintf(stderr, __VA_ARGS__); } while(0)
#else
